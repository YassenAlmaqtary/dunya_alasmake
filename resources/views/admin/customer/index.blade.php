...
@section('page_title2')
<a href="{{route('admin.customer.create')}}"> اضافة عميل</a>
@endsection
      ...
      <div class="modal-dialog">
        <div class="modal-content bg-danger">
          ...
          <div class="modal-footer justify-content-between">
            ...
            <button type="button" id="save-chaing" onclick="deletItem('{{route('admin.customer.delete')}}',event)" class="btn btn-outline-light">حفظ التغيرات</button>
          </div>
        </div>
        <!-- /.modal-content -->
      </div>
            ...
            <table id="example1" class="table table-bordered table-striped">
              ...
              <tbody>
                  ...
                  <tr>
                      ...
                      <td>{{$customer->name}}</td>
                      <td><img style="width: 50px; height: 50px;" src="{{$customer->image_path}}"></td>
                      ...
                      <td>
                          ...
                          <a class="btn btn-info btn-sm" href="{{route('admin.customer.edit',$customer->id)}}">
                            <i class="fas fa-pencil-alt">
                            </i>
                            تعديل
                          </a>
          
                        <a class="btn btn-danger btn-sm" href="#" onclick="setIDItem({{$customer->id}})" data-toggle="modal" data-target="#modal-danger">
                          <i class="fas fa-trash">
                          </i>
                          حذف
                         </a>
                    
                          <input type="checkbox" onchange="clickFn({statu:`${statu=this.checked?'1':'0'}`,customer_id:`${this.value}`},
                          '{{route('customerStatus')}}')" value="{{$customer->id}}" data-toggle="switchbutton" data-onlabel="مفعل" data-offlabel="غير مفعل"@if ($customer->status)
                           checked 
                           @endif 
