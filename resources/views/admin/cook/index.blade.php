...
@section('page_title2')
<a href="{{route('admin.cook.create')}}">اضافة طبخة</a>
@endsection
      ...
      <div class="modal-dialog">
        <div class="modal-content bg-danger">
          ...
          <div class="modal-footer justify-content-between">
            ...
            <button type="button" id="save-chaing" onclick="deletItem('{{route('admin.cook.delete')}}',event)" class="btn btn-outline-light">حفظ التغيرات</button>
          </div>
        </div>
        <!-- /.modal-content -->
      </div>
        ...
        <table id="example1" class="table table-bordered table-striped">
          ...
          <tbody>
              ...
              <tr>
                  ...
                  <td>{{$cook->name}}</td>
                  <td><img style="width: 50px; height: 50px;" src="{{$cook->image_path}}"></td>
                  <td>{{$cook->price}}</td>
                  <td style="width:20%;height:5%;">{{$cook->details}}</td>
                  {{-- <td>{{$product->discount_price}}</td> --}}
                  
                  <td style="width:30%;">
                  <a class="btn btn-info btn-sm" href="{{route('admin.cook.edit',$cook->id)}}">
                    <i class="fas fa-pencil-alt">
                    </i>
                    تعديل
                  </a>
  
                <a class="btn btn-danger btn-sm" href="#" onclick="setIDItem({{$cook->id}})" data-toggle="modal" data-target="#modal-danger">
                  <i class="fas fa-trash">
                  </i>
                  حذف
                 </a>
            
                  <input type="checkbox" onchange="clickFn({statu:`${statu=this.checked?'1':'0'}`,cook_id:`${this.value}`},
                  '{{route('cookStatus')}}')" value="{{$cook->id}}" data-toggle="switchbutton" data-onlabel="مفعل" data-offlabel="غير مفعل"@if ($cook->status)
                   checked 
                   @endif 
